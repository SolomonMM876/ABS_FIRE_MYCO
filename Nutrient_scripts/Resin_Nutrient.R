...
Bag_Site_notes <- Bag_Site_notes %>%
  mutate(
    ...
    Res_nute_sub_w = as.numeric(Res_nute_sub_w),
...
NO3_NH4_Resin<-bind_rows(NH4NO3_RESIN_LOT_1,NH4NO3_RESIN_LOT_2)[,-c(10:13)] %>%
 ...
 filter(!str_detect(Sample, 'STAND|C C|CC|NIRAJ'))%>%
  ...
  rename(Ammonia= 'Result_Ammonia 2.0',
         ...
         Ammonia_Auto_Dil = `Auto Dil_Ammonia 2.0`)%>%
  ...
  mutate(#calc blank avg for Nitr
    ...
    Ammonia_mg_kg= Ammonia *(7.5/Res_nute_sub_w),
    #Amm_bad=Ammonia *(7.5/Res_remain_w),
    ...
    Nitrate_mg_kg= Nitrate*(7.5/Res_nute_sub_w),
    #Nitr_bad= Nitrate *(7.5/Res_remain_w)
    )#7.5mL used for nutrient subset
...
p<-NO3_NH4_Resin%>%
  ...
  ggplot(aes(x=interaction(Transect,Location) ))+
  geom_col(aes(y=Ammonia_mg_kg),fill='blue', position = "dodge") +
#  geom_col(aes(y=Amm_bad),fill='grey', position = "dodge") +
...
p<-NO3_NH4_Resin%>%
  ...
  arrange(Site,Transect,Location)%>%
  ggplot(aes(x=interaction(Transect,Location) ))+
  geom_col(aes(y=Nitrate_mg_kg),fill='blue', position = "dodge") +
 # geom_col(aes(y=Nitr_bad),fill='grey', position = "dodge") +
  facet_grid(~Site,  scales = "free_x", )+
...
Ortho_P_Resin<-bind_rows(OPHOS_RESIN_HCL_LOT_1,OPHOS_RESIN_HCL_LOT_2) %>%
  ...
  mutate(Blank_avg = mean(Result[grepl("CTRL", Site)], na.rm = TRUE), 
         ...
         Ortho_P_mg_kg= Ortho_blanked *(7.5/Res_nute_sub_w),
         #Ortho_P_bad= Ortho_blanked *(7.5/Res_remain_w)
         )#7.5mL used for 1 g of resin
...
p<-Ortho_P_Resin%>%
  ...
  arrange(Site,Transect,Location)%>%
  ggplot(aes(x=interaction(Transect,Location) ))+
  geom_col(aes( y=Ortho_P_mg_kg),fill='blue', position = "dodge") +
 # geom_col(aes(y=Ortho_P_bad),fill='grey', position = "dodge") +
  facet_grid(~Site,  scales = "free_x", )+
...
Bag_data<-read.csv('Processed_data/Updated_Bag_data.csv')%>% distinct(Site,Fire.Interval,Fire.Severity) %>% mutate(
  Site=as.factor(Site)
)


Nutrient_Resins<-Nutrient_Resins%>%group_by(Site,Transect,Location)%>%#merging technical replicates for Site 29 T2 L16
  summarise(Ortho_P_mg_kg=mean(Ortho_P_mg_kg),
            Ammonia_mg_kg= mean(Ammonia_mg_kg),
            Nitrate_mg_kg=mean(Nitrate_mg_kg)) %>% 
  left_join(Bag_data)


write.csv(Nutrient_Resins, 'Processed_data/Resin_Nutrients.csv',row.names = FALSE)
