...
#Nutrient resins
Resin_Nutrients<-read.csv('Processed_data/Resin_Nutrients.csv')
...
Total_P<-Manual_Calc_Total_P%>%
  ...
  Percent_Phos_ = ifelse(Percent_Phos_ < 0 & Sample_mg > 1.1, 0.002, Percent_Phos_)) #lowest value on standard curve replaces neg value
...
CNP_clean<-CNHP%>%
  ...
  # Sample is believed to contain 'other' non C/N material given the relatively low concentrations found
  filter(Sample!='56.2')%>%
  filter( !Fire.Interval=='Standards')%>%
  mutate(Percent_Phos_ = if_else(Sample_mg_P < 1, NA, Percent_Phos_))%>%
  select(-ID,-Stan_Group)%>%
  mutate(C_P = if_else(is.na(Percent_Phos_), NA, Carbon / Percent_Phos_),
         N_P= Nitrogen/Percent_Phos_,
         C_N_P_ratio = case_when( is.na(Percent_Phos_)~ NA,
                                  TRUE~ paste(round(C_P, 0), round(N_P, 0), "1", sep = ":")))
# Overall Summary
CNP_Summary <- CNP_clean %>%
  summarise(Carbon = mean(Carbon, na.rm = TRUE),
            Nitrogen = mean(Nitrogen, na.rm = TRUE),
            Percent_Phos_ = mean(Percent_Phos_, na.rm = TRUE),
            C_N = mean(C_N, na.rm = TRUE),
            C_P = mean(C_P, na.rm = TRUE),
            N_P = mean(N_P, na.rm = TRUE),
            C_N_P_ratio = paste(round(C_P, 0), round(N_P, 0), "1", sep = ":")) %>%
  mutate(Sample = "Summary", Fire.Interval = "Overall", Fire.Severity = "Overall")

# #excluding extremely low P sample outliers
# CNP_Summary_no_out <- CNP_clean %>%
#   summarise(Carbon = mean(Carbon, na.rm = TRUE),
#             Nitrogen = mean(Nitrogen, na.rm = TRUE),
#             Percent_Phos_ = mean(Percent_Phos_, na.rm = TRUE),
#             C_N = mean(C_N, na.rm = TRUE),
...
#             N_P = mean(N_P, na.rm = TRUE),
#             C_N_P_ratio = paste(round(C_P, 0), round(N_P, 0), "1", sep = ":")) %>%
#   mutate(Sample = "Summary 
#          (outliers excluded)", Fire.Interval = "Overall (outliers excluded)", Fire.Severity = "Overall (outliers excluded)")

# Summary by Fire Interval
CNP_Summary_Interval <- CNP_clean %>%
  ...
  summarise(Carbon = mean(Carbon, na.rm = TRUE),
            ...
            N_P = mean(N_P, na.rm = TRUE),
            C_N_P_ratio = paste(round(C_P, 0), round(N_P, 0), "1", sep = ":")) %>%
  mutate(Sample = "Summary", Fire.Severity = "By Interval")

# Summary by Fire Severity
CNP_Summary_Severity <- CNP_clean %>%
  ...
  summarise(Carbon = mean(Carbon, na.rm = TRUE),
            ...
            N_P = mean(N_P, na.rm = TRUE),
            C_N_P_ratio = paste(round(C_P, 0), round(N_P, 0), "1", sep = ":")) %>%
  mutate(Sample = "Summary", Fire.Interval = "By Severity")

# Ensure all columns match before binding
summary_dfs <- list(CNP_Summary, CNP_Summary_Interval, CNP_Summary_Severity)

# Standardize columns
summary_dfs <- lapply(summary_dfs, function(df) {
  df %>%
    mutate(Transect = NA, Site = NA) %>% # Fill missing cols
    relocate(Site, Transect, Sample, Fire.Interval, Fire.Severity)
})

# Bind the original data with summaries at the bottom
CNP_final <- bind_rows(CNP_clean, summary_dfs)
...
CNP_summary_SE <- CNP_clean %>%
  summarise(
    mean_Carbon = mean(Carbon, na.rm = TRUE),
    se_Carbon = sd(Carbon, na.rm = TRUE) / sqrt(n()),
    
    mean_Nitrogen = mean(Nitrogen, na.rm = TRUE),
    se_Nitrogen = sd(Nitrogen, na.rm = TRUE) / sqrt(n()),
    
    mean_Hydrogen = mean(Hydrogen, na.rm = TRUE),
    se_Hydrogen = sd(Hydrogen, na.rm = TRUE) / sqrt(n()),
    
    mean_C_N = mean(C_N, na.rm = TRUE),
    se_C_N = sd(C_N, na.rm = TRUE) / sqrt(n()),
    
    mean_C_P = mean(C_P, na.rm = TRUE),
    se_C_P = sd(C_P, na.rm = TRUE) / sqrt(n()),
    
    mean_N_P = mean(N_P, na.rm = TRUE),
    se_N_P = sd(N_P, na.rm = TRUE) / sqrt(n())
  )

print(CNP_summary_SE)
...
write.csv(CNP_final, "Stoich_CNP/CNP_final.csv",row.names = FALSE)
