...
modelDir = file.path("HMSC_ABS/models_updated")
library(Hmsc)
set.seed(1)
...
samples <- 500
nChains <- 2
filename=file.path(modelDir, paste0("models_thin_",as.character(thin),"_samples_",as.character(samples),"_chains_",as.character(nChains),'1.Rdata'))
load(filename)

thin <- 20
samples <- 500
nChains <- 2
filename=file.path(modelDir, paste0("models_thin_",as.character(thin),"_samples_",as.character(samples),"_chains_",as.character(nChains),'2.Rdata'))
load(filename)
...
save(models, file='HMSC_ABS/models_updated/interaction_model_thin_20_samples_500_Chains_2.RData')
...
resultDir = file.path(localDir, "results_updated")
##################################################################################################
# SET DIRECTORIES (END)
##################################################################################################
...
while(Lst <= nst){
  thin = thin
  samples = samples
  filename= file.path('HMSC_ABS/models_updated/updated_model_thin_20_samples_300_Chains_2.RData')
