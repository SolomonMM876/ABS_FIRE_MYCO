...
# Convert model output to coda object
mpost = convertToCodaObject(models[[1]], spNamesNumbers = c(TRUE, FALSE), covNamesNumbers = c(TRUE, FALSE))

# Create a cluster with available cores
numCores <- detectCores()  # Use all but one core
cl <- makeCluster(numCores)

# Export necessary variables and functions to the cluster
clusterExport(cl, varlist = c("mpost", "gelman.diag"))

# Run Gelman diagnostics in parallel
psrf.list = parLapply(cl, list(mpost$Beta, mpost$Omega[[1]]), function(x) gelman.diag(x, multivariate=FALSE)$psrf)

# Stop the cluster
stopCluster(cl)

# Extract results
psrf.beta = psrf.list[[1]]
psrf.omega = psrf.list[[2]]

# Use single plotting window for efficiency
