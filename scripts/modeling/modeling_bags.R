...
m_biomass_day_resins<-lmer(Biomass_day_log~Fire.Severity+Fire.Interval +
...
emm_Interval <- emm_biomass_Interval %>%
  mutate(
    emmean_bt = 10^emmean*(1e+06/15)* (150/1000),
    lower.CL_bt = 10^lower.CL*(1e+06/15)* (150/1000),
    upper.CL_bt = 10^upper.CL*(1e+06/15)* (150/1000)
  )
emm_Interval
...
p<-ggplot(emm_biomass_Interval, aes(x = Fire.Interval, y = (10^(emmean)*(1e+06/15)* (150/1000))) )+
  geom_col(aes(fill=Fire.Interval),size=4, width = .7) +
  geom_jitter(data=Bag_data, aes(x=Fire.Interval, y=(Biomass_day)*(1e+06/15)* (150/1000)), size=3, width=.1, alpha=.6)+
  geom_errorbar(aes(ymin = ((10^lower.CL)*(1e+06/15)* (150/1000)),
                    ymax = ((10^upper.CL)*(1e+06/15)* (150/1000)), width = 0.2 ),
                width= .4, size= 1.5) +
  ...
  scale_y_continuous(breaks = seq(0, 2300, by = 50)) +
...
p<-ggplot(Bag_data, aes(x = Fire.Interval, y = biomass_kg_ha_day* (150) ))+
  geom_boxplot(aes(fill=Fire.Interval),linewidth=1.5, width = .7, outliers = FALSE) +
  geom_jitter(data=Bag_data, aes(x=Fire.Interval, y=biomass_kg_ha_day* (150)), size=3, width=.1, alpha=.6)+
  labs(x = "Fire Frequency", y = "Hyphal Production (kg/ha) over 5 months") +
  scale_fill_manual(values = interval_colors) +     # Custom colors for Interval
  scale_y_continuous(breaks = seq(0, 2300, by = 50)) +
  annotate("text", x =1.75, y = Inf, label = paste0("Fire Frequency (p) = ", Anova_resin["Fire.Interval", "Pr(>F)"]),
...
#Ortho P and Biomass
library(ggeffects)
ortho_bio<-predict_response(m_biomass_day_resins, terms = c('Ortho_P_mg_kg'), back_transform = FALSE) %>%
  mutate(
    Biomass_day = (10^(predicted) * (1e+06 / 15)),
    confidence.low = (10^(conf.low) * (1e+06 / 15)),
    confidence.high = (10^(conf.high) * (1e+06 / 15))
  ) %>% as.data.frame() %>% 
  ggplot(aes(x = x, y = Biomass_day)) +  # use correct x column name
  geom_line(color = "black", linewidth = 1.5) +
  geom_ribbon(aes(ymin = confidence.low, ymax = confidence.high), alpha = 0.1) +
  geom_point(
    data = Bag_data,
    mapping = aes(x = Ortho_P_mg_kg, y = (10^(Biomass_day_log) * (1e+06 / 15))),
    size = 2.5
  ) +
  labs(
    x = expression(paste(PO[4], " (mg/kg)")),
    y = "Hyphal Production (kg/ha) over 5 months"
  ) +
  annotate(
    "text",
    x = 0, y = Inf,
    label = paste0("p = ", Anova_resin$`Pr(>F)`[3]),
    hjust = 0.1, vjust = 1.1, size = 6
  ) +
  scale_y_log10()+
...
ggsave(
  filename = "plots/Ortho_biomass.png",
  plot = ortho_bio,
  device = "png",
  width = 20,          # double-column width
  height = 13,         # proportionate height
  units = "cm"
)
...
# #host
# 
# hist(Bag_data$perc_myco_host_freq)
# m4<-lmer((perc_myco_host_freq) ~   Fire.Interval + Fire.Severity  + (1|Site) , 
#          data=Bag_data%>%select(perc_myco_host_freq,Fire.Interval,Fire.Severity,Site) %>% distinct())
# #perc_N_fix_freq
# 
# 
# #3 (NH4~Regime) Not Sig
...
# hist(Bag_data$perc_myco_host_freq)
# m4<-lmer((Tree.Basal.Area_m2) ~   Fire.Interval + Fire.Severity  + (1|Site) , 
#          data=Bag_data%>%select(Tree.Basal.Area_m2,Fire.Interval,Fire.Severity,Site,Transect) %>% distinct())
# 
# #3 (NH4~Regime) Not Sig
# summary(m4)
# Anova(m4,test='F')
...
# r2(m4)
# 
# m4<-lmer((All.Tree.Canopy.Cover_perc) ~   Fire.Interval + Fire.Severity  + (1|Site) , 
#          data=Bag_data%>%select(Tree.Basal.Area_m2,Fire.Interval,Fire.Severity,Site) %>% distinct())
# 
# #3 (NH4~Regime) Not Sig
# summary(m4)
# Anova(m4,test='F')
...
Bag_data%>%
  ggplot(aes(x=Site, y=biomass_kg_ha_day))+
...
Bag_data %>%
  ggplot(aes(y = biomass_kg_ha_day,
             x = Fire.Interval)) +
...
Bag_data %>%
  ggplot(aes(y = biomass_kg_ha_day, x = Site,)) +
...
Bag_data %>%
  ggplot(aes(y = biomass_kg_ha_day + 0.095, x = Site, fill = Fire.Interval)) +
