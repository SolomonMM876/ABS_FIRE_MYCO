...
#calculate number of bags collected
temp<-bag_data %>% 
  filter(is.na(Missing)|!Missing=='y')
...
write.csv(bag_data, file='Processed_data/bag_data.csv', row.names = FALSE)
...
site_data%>%ungroup()%>%select(Site,Days_Installed)%>%distinct()%>%summarise(mean_days=mean(Days_Installed))
sd(site_data$Site)
site_data %>%ungroup()%>%
  summarise(
...
Bag_Site <- left_join(corrected_myc,site_data) %>%
  mutate(
    ...
    biomass_g_ha_day = Biomass_day * (1e+06 / 30),  # Convert to g/ha/day see lines below
  )
#g/hectare:
#10,000 m^2 ×0.1 m= 1,000 m^3 
#1,000m^3 x (x mg /(15cm^3)*2 bags ) x (1g/1000mg) x 1000kg/ton = g/ha
#(1e+06/30)

# Summary of biomass data
Bag_Site %>% ungroup() %>% 
  #group_by(Site)%>%
  summarise(all_mean = mean(biomass_g_ha_day, na.rm = TRUE)* 150/1000,
            min_biomass = min(biomass_g_ha_day, na.rm = TRUE),
            max_biomass = max(biomass_g_ha_day, na.rm = TRUE),
            min_150kg = min_biomass * 150/1000,
            max_150kg = max_biomass * 150/1000,
            min_365kg = min_biomass * 365/1000,
            max_365kg = max_biomass * 365/1000
            )%>%arrange(desc(min_150kg))
...
Bag_Site<-Bag_Site%>%
  ...
  filter(!is.na(myc_2nd_w))#no biomass data for these locations due to labelling errors had to exclude
...
Bag_Guild<-Bag_Site  %>% 
  left_join(guild_summary%>% 
              mutate(Site=as.factor(Site),Transect= as.factor(Transect), 
                     Location=as.factor(Location),Tube_ID=as.factor(Tube_ID))) %>% 
...
Bag_Guild %>% ungroup() %>% 
  ...
  summarise(
    n = sum(!is.na(biomass_g_ha_day)),
    all_mean = mean(biomass_g_ha_day, na.rm = TRUE) * 150 / 1000,
    se = (sd(biomass_g_ha_day, na.rm = TRUE) / sqrt(n)) * 150 / 1000,
    min_biomass = min(biomass_g_ha_day, na.rm = TRUE),
    max_biomass = max(biomass_g_ha_day, na.rm = TRUE),
    min_150kg = min_biomass * 150 / 1000,
    max_150kg = max_biomass * 150 / 1000
...
Site.variation <- Bag_Site %>%
  ...
  left_join(VEG_Transect %>%  select(Site,Transect, Fire.Interval, Fire.Severity))
...
Bag_Site %>%
  ...
  labs(x = "Fire Interval", y = "gs per hectare per day") +
...
# Plot prodcution by fire interval and biomass
interval_colors <- c("Long" = "darkred", "Short" = "orange")
Site.variation %>%
  ...
  theme_classic() +
  theme(
    axis.text.x = element_text(hjust = 1, angle=45,size = 25, face = "bold"),
    axis.text.y = element_text(size = 23, face = "bold"),
    axis.title.x = element_text(size = 30, face = "bold"),
    axis.title.y = element_text(size = 27, face = "bold"),
    axis.line = element_line(linewidth = 1.5),
...
# ------------------------------------------------------------
# Summarise biomass per location
# ------------------------------------------------------------
Bag_harvest<-Bag_Site %>%
  #group_by(Site, Transect, Location) %>%
  ungroup() %>% 
  summarise(
    n_bags = sum(!is.na(myc_2nd_w)),
    total_biomass_harvested = sum(myc_2nd_w, na.rm = TRUE),
    mean_biomass_harvested = mean(myc_2nd_w, na.rm = TRUE),
    min_biomass_harvested = min(myc_2nd_w, na.rm = TRUE),
    max_biomass_harvested = max(myc_2nd_w, na.rm = TRUE),
    range_biomass_harvested = max_biomass_harvested - min_biomass_harvested
  ) %>%
  ungroup()
