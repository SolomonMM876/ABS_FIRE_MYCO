...
sim_res <- simulateResiduals(fittedModel = plant_path_model_fire)
plot(sim_res)
#plot(explo_model_fire)
qqPlot(resid(plant_path_model_fire))
summary(plant_path_model_fire)
Anova_resin<-round(Anova(plant_path_model_fire,test='F'), 2) 
Anova_resin


emm_severity <- as.data.frame(emmeans(plant_path_model_fire, ~ Severity))
emm_interval <-as.data.frame(emmeans(plant_path_model_fire, ~ Interval))
