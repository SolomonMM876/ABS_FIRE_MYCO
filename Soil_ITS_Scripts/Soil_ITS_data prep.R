...
tax <- otu %>%
  ...
  merge(FunGuildData_org,
    ...
    filter(confidenceRanking == "Highly Probable" | SH_ID %in% c("SH1243176.09FU",'SH0879828.09FU',
                                                                 'SH1258648.09FU','SH1126152.09FU','SH1275469.09FU')) %>%  # Keep "Highly Probable" & specified OTUs
  filter(!str_detect(guild, "-") | SH_ID %in% c("SH1243176.09FU",'SH0879828.09FU',
                                                'SH1258648.09FU','SH1126152.09FU','SH1275469.09FU')) %>%  # Exclude multiple guilds except for specified OTUs
  mutate(
    guild2 = case_when(
...
write.csv(dat_myco_RA_soil, 'Processed_data/Soil_ITS_myco_data.csv',row.names = FALSE)
...
# Step 1: Calculate total reads per sample (already in `reads_samp`, so just distinct it)
total_reads_per_sample <- dat_myco_RA_soil %>%
  distinct(sample_ID, reads_samp)

# Step 2: Filter for Thelephora and sum readcount per sample
thelephora_reads <- dat_myco_RA_soil %>%
  filter(Genus == "Lactarius") %>%
  group_by(sample_ID,Site) %>%
  summarise(thelephora_reads = sum(readcount), .groups = "drop")

# Step 3: Join and calculate percent
thelephora_percent <- total_reads_per_sample %>%
  left_join(thelephora_reads, by = "sample_ID") %>%
  mutate(
    thelephora_reads = replace_na(thelephora_reads, 0),
    percent_thelephora = (thelephora_reads / reads_samp) * 100
  )

# View result
print(thelephora_percent)
