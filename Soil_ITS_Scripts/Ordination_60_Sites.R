...
p<-dat_ecm%>% select(ends_with('.09FU'))%>% 
  pivot_longer(cols=ends_with('.09FU'), names_to='SH_ID', 
...
Interval_Indicator<-out_Interval %>% 
  ggplot(aes(x=Interval, y=(count/p)*100, fill=Genus, text=SH_ID)) + # text aesthetic is for the ggplotly visualisation below
...
adonis2(dat_ecm_otu ~ Severity+Interval+ Veg_Class + perc_myco_host_freq  ,
...
cap1 <- capscale(dat_ecm_otu ~Severity+Interval+ Veg_Class + perc_myco_host_freq 
                 , data=dat_ecm, distance='robust.aitchison', add=TRUE)
...
vp <- varpart(vegdist(mat_ecm, distance='robust.aitchison'), 
              ...
              ~ Shrub.Cover_50.200cm_perc + Tree.Basal.Area_m2+perc_myco_host_freq , #Vegetation
