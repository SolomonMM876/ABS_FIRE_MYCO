...
guild_abundance <- dat_all_RA_soil %>%
  group_by(Site,Transect, Fire.Interval, Fire.Severity, guild2) %>%
  summarise(total_abundance = sum(readcount, na.rm = TRUE), .groups = "drop") %>%
...
cor_results <- guild_abundance %>%
  ...
  summarise(correlation = cor(Ectomycorrhizal, Saprotroph, method = "pearson", use = "complete.obs"),
...
model_Interval<- lmer(log10(Ectomycorrhizal) ~ Saprotroph * Fire.Interval +(1/Site|Transect), data = guild_abundance)

model_Severity<- lmer(log10(Ectomycorrhizal) ~ Saprotroph * Fire.Severity +(1/Site|Transect), data = guild_abundance)

model_Regime<- lmer(log10(Ectomycorrhizal) ~ Saprotroph * (Fire.Interval + Fire.Severity) +(1/Site|Transect), data = guild_abundance)

model_Regime<- lmer(log10(Ectomycorrhizal) ~ `Arbuscular Mycorrhizal` * (Fire.Interval) +(1/Site|Transect), data = guild_abundance)
...
ggplot(guild_abundance, aes(x = Saprotroph, y = Ectomycorrhizal, color = Fire.Interval)) +
  geom_point() +
