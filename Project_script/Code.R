###############################################################################
# Script: Decoupled responses of mycorrhizal fungal communities and function
...
# Load split files
site_rnd1 <- read.csv('data/site_data_Rnd1.csv')
taxa_rnd1 <- read.csv('data/taxa_data_Rnd1.csv')

# Merge metadata with taxa data
Data_rnd1 <- left_join(site_rnd1, taxa_rnd1, by = "Tube_ID")

Data_rnd1 <- Data_rnd1 %>%
  mutate(
    # Remove biomass from rows where seq data suggests non-myco taxa dominate
    ...
    # Convert biomass to kg/ha/day:
    # (10,000 m^2 * 0.1 m) * (mg hyph / (15cm^3) * 2 bags) * (1g/1000mg) * 1000kg/ton
...
# Load split files
site_rnd2 <- read.csv('data/site_data_Rnd2.csv')
taxa_rnd2 <- read.csv('data/taxa_data_Rnd2.csv')

# Merge metadata with taxa data
Data_rnd2 <- left_join(site_rnd2, taxa_rnd2, by = "Tube_ID")
...
########################
# 3. Effect of Fire Regime on Nutrients
########################
...
Nutrient_Anova %>% arrange(Factor)

########################
# 4. Mycorrhizal Community Analyses
########################

## 4.1 Alpha Diversity – First Round
...
## 4.2 Beta Diversity – First Round
...
## 4.3 CAP Ordination – First Round
...
# Saving plots (commented out for review)
# ggsave(filename = "plots/submission/Figure_2.pdf", plot = p3, device = "pdf", width = 18, height = 12, units = "cm")
# ggsave(filename = "plots/submission/Figure_2.png", plot = p3, device = "png", width = 18, height = 12, units = "cm")


########################
# 5. Mycorrhizal Biomass
########################
...
########################
# 7. Second Mycelial Collection Analyses
########################
...
qqPlot(resid(m_biomass), main = "QQ Plot: Biomass Model (2nd Rnd)")
...
run_alpha_model <- function(metric, data) {
  ...
  print(summary(model))
  # Note: qqPlot may stop script execution in some environments if not interactive
...
## 7.3 Beta Diversity (2nd Round)

# Filter out samples with no mycorrhizal reads
Data_rnd2_clean <- Data_rnd2 %>%
  filter(!if_all(starts_with("ITSall"), ~ . == 0))

# Extract community matrix
mat_myco <- Data_rnd2_clean %>% select(starts_with("ITSall"))

# PERMANOVA
adonis_rnd2 <- adonis2(mat_myco ~ Fire.Interval, data = Data_rnd2_clean,
...
# CAP Ordination
cap_model <- capscale(mat_myco ~ Fire.Interval, data = Data_rnd2_clean,
                      distance = "robust.aitchison", add = TRUE)
anova(cap_model)
summary(cap_model)

# Plotting 2nd Round CAP
...
p2 <- cbind(Data_rnd2_clean, scrs_site) %>%
